...
class LangGraphAgent:
    ...
    async def get_checkpoint_before_message(self, message_id: str, thread_id: str):
        ...
        # Handle synthetic reasoning message IDs (format: "{original_id}-reasoning-{index}")
        # These IDs are generated by langchain_messages_to_agui() but don't exist in
        # LangGraph checkpoint history - we need to use the original AIMessage ID
        lookup_id = message_id
        if "-reasoning-" in message_id:
            # Extract the original AIMessage ID
            lookup_id = message_id.rsplit("-reasoning-", 1)[0]
        ...
        for idx, snapshot in enumerate(history_list):
            ...
            if any(getattr(m, "id", None) == lookup_id for m in messages):
                if idx == 0:
