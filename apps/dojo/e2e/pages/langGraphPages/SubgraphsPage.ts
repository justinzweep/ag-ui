...
export class SubgraphsPage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.travelPlannerButton = page.getByRole("button", {
      name: /travel.*planner|subgraphs/i,
    });
    this.agentGreeting = page.getByText(
      /travel.*planning|supervisor.*coordinate/i,
    );
    ...
    this.flightOptions = page.locator(
      '[data-testid*="flight"], .flight-option',
    );
    ...
    this.hotelZephyrOption = page.getByText(
      /Hotel Zephyr.*Fisherman\'s Wharf.*\$280/,
    );
    ...
    this.hotelSelectionInterface = page.locator(
      '[data-testid*="hotel-select"], .hotel-selection',
    );

    // Itinerary elements
    this.itineraryDisplay = page.locator(
      '[data-testid*="itinerary"], .itinerary',
    );
    ...
    this.experienceRecommendations = page.locator(
      '[data-testid*="experience"], .experience',
    );

    // Agent activity indicators
    this.activeAgent = page.locator(
      '[data-testid*="active-agent"], .active-agent',
    );
  ...
  async verifyStaticExperienceData() {
    ...
    await expect(this.page.getByText("No experiences planned yet"))
  ...
  async verifyItineraryContainsHotel(hotel: "Zephyr" | "Ritz-Carlton" | "Zoe") {
    ...
    const hotelName =
      hotel === "Ritz-Carlton" ? "Ritz-Carlton" : `Hotel ${hotel}`;
  ...
  async waitForAgentCompletion() {
    ...
    await expect(
      this.page
        .getByText(/complete|finished|planning.*done|itinerary.*ready/i)
        .first(),
